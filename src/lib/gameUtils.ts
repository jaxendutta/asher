...
export function createPlayer(): PIXI.Container {
    ...
    if (mainCharTexture) {
        ...
        // Assuming 16x16 pixel frames (adjust if your sprites are different size)
        const frameWidth = 16;
        const frameHeight = 16;

        // Get the first frame (facing down, standing still)
        // This is typically at position (0, 0) in Pokemon-style sprite sheets
        const singleFrameTexture = new PIXI.Texture({
            source: mainCharTexture.source,
            frame: new PIXI.Rectangle(0, 0, frameWidth, frameHeight),
        });

        const sprite = new PIXI.Sprite(singleFrameTexture);
        ...
        // Store texture info for animation later
        (container as any).baseTexture = mainCharTexture;
        (container as any).frameWidth = frameWidth;
        (container as any).frameHeight = frameHeight;
    } else {
...
export function createBook(bookNumber: number = 1): PIXI.Container {
    ...
    if (bookTexture) {
        ...
        sprite.scale.set(3); // Make books bigger - 3x scale
        container.addChild(sprite);
    } else {
