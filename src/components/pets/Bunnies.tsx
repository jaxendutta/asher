"use client";

import React, { useEffect, useRef, useState, useCallback } from 'react';
import { tiny5 } from '@/lib/fonts';

interface Position {
    x: number;
    y: number;
}

interface Sprite {
    el: HTMLDivElement | null;
    x: number;
    y: number;
}

interface Actor {
    id: string;
    x: number;
    y: number;
    frameOffset: number;
    animationTimer: NodeJS.Timeout | null;
    sprite: Sprite;
    buffer: number;
    move: Position;
    pause?: boolean;
    walkingInterval?: NodeJS.Timeout | null;
    sad?: boolean;
}

const Bunnies: React.FC = () => {
    const mapRef = useRef<HTMLDivElement>(null);
    const playerRef = useRef<HTMLDivElement>(null);
    const radarRef = useRef<HTMLDivElement>(null);

    const [sadBunnyCount, setSadBunnyCount] = useState(20);
    const [showEndMessage, setShowEndMessage] = useState(false);
    const [radarSize, setRadarSize] = useState(0);
    const [mapPos, setMapPos] = useState({ x: 0, y: 0 });
    const [bunnies, setBunnies] = useState<Actor[]>([]);
    const [radarBunnies, setRadarBunnies] = useState<Array<{ angle: number; distance: number; id: string }>>([]);

    const settingsRef = useRef({
        d: 20,
        offsetPos: { x: 0, y: 0 },
        map: {
            w: 20 * 200,
            h: 20 * 200,
            x: 0,
            y: 0,
        },
        isWindowActive: true,
        controlPos: { x: 0, y: 0 },
    });

    const playerRef2 = useRef<Actor>({
        id: 'bear',
        x: 0,
        y: 0,
        frameOffset: 1,
        animationTimer: null,
        sprite: { el: null, x: 0, y: 0 },
        buffer: 20,
        move: { x: 0, y: 0 },
        pause: false,
        walkingInterval: null,
    });

    const randomN = (max: number) => Math.ceil(Math.random() * max);
    const px = (n: number) => `${n}px`;
    const distanceBetween = (a: Position, b: Position) =>
        Math.round(Math.sqrt(Math.pow(a.x - b.x, 2) + Math.pow(a.y - b.y, 2)));
    const radToDeg = (rad: number) => Math.round(rad * (180 / Math.PI));

    const setBackgroundPos = (sprite: Sprite) => {
        if (sprite.el) {
            sprite.el.style.setProperty('--bx', px(sprite.x));
            sprite.el.style.setProperty('--by', px(sprite.y));
        }
    };

    const animateSprite = (actor: Actor, dir: string) => {
        const h = -32 * 2;
        actor.sprite.y = {
            down: 0,
            up: h,
            right: h * 2,
            left: h * 3,
        }[dir] || 0;
        actor.frameOffset = actor.frameOffset === 1 ? 2 : 1;
        actor.sprite.x = actor.frameOffset * (2 * -20);
        setBackgroundPos(actor.sprite);
    };

    const stopSprite = (actor: Actor) => {
        actor.sprite.x = 0;
        setBackgroundPos(actor.sprite);
        if (actor.walkingInterval) {
            clearInterval(actor.walkingInterval);
            actor.walkingInterval = null;
        }
    };

    const getRandomPos = (key: 'w' | 'h') =>
        20 * randomN((settingsRef.current.map[key] / 20) - 1);

    const createBunny = useCallback((): Actor => {
        return {
            id: `bunny-${Date.now()}-${Math.random()}`,
            x: getRandomPos('w'),
            y: getRandomPos('h'),
            frameOffset: 1,
            animationTimer: null,
            sprite: { el: null, x: 0, y: 0 },
            sad: true,
            buffer: 30,
            move: { x: 0, y: 0 },
        };
    }, []);

    const triggerBunnyWalk = useCallback((bunny: Actor) => {
        if (bunny.animationTimer) clearInterval(bunny.animationTimer);

        bunny.animationTimer = setInterval(() => {
            if (!settingsRef.current.isWindowActive) return;
            const dir = ['up', 'down', 'right', 'left'][Math.floor(Math.random() * 4)];
            const { d } = settingsRef.current;
            bunny.move = {
                down: { x: 0, y: d },
                up: { x: 0, y: -d },
                right: { x: d, y: 0 },
                left: { x: -d, y: 0 },
            }[dir] || { x: 0, y: 0 };

            animateSprite(bunny, dir);
            setTimeout(() => animateSprite(bunny, dir), 300);
            setTimeout(() => animateSprite(bunny, dir), 600);
            setTimeout(() => stopSprite(bunny), 900);
        }, 2000);
    }, []);

    const noWall = useCallback((actor: Actor, testBunnies: Actor[]) => {
        const newPos = { ...actor };
        newPos.x += actor.move.x;
        newPos.y += actor.move.y;

        if (testBunnies.some(el =>
            el.id !== actor.id &&
            distanceBetween(el, newPos) <= el.buffer &&
            distanceBetween(el, actor) > el.buffer
        )) return false;

        const buffer = 40;
        const noWallX = actor.move.x > 0
            ? newPos.x + buffer < settingsRef.current.map.w
            : newPos.x - buffer > 0;
        const noWallY = actor.move.y > 0
            ? newPos.y < settingsRef.current.map.h - buffer
            : newPos.y - buffer > 0;
        return noWallX && noWallY;
    }, []);

    const walk = useCallback((actor: Actor, dir: string, testBunnies: Actor[]) => {
        if (!dir || actor.pause || !settingsRef.current.isWindowActive) return;
        if (noWall(actor, testBunnies)) {
            animateSprite(actor, dir);
            actor.x += actor.move.x;
            actor.y += actor.move.y;
        } else {
            stopSprite(actor);
        }
    }, [noWall]);

    const hugBunny = useCallback((bunny: Actor, bunnyIndex: number) => {
        const player = playerRef2.current;
        const classToAdd = bunny.x > player.x ? 'hug-bear-bunny' : 'hug-bunny-bear';

        if (bunny.animationTimer) clearInterval(bunny.animationTimer);
        player.pause = true;
        bunny.sad = false;
        player.y = bunny.y;

        if (classToAdd === 'hug-bear-bunny') {
            player.x = bunny.x - 40;
            animateSprite(player, 'right');
            animateSprite(bunny, 'left');
        } else {
            player.x = bunny.x + 40;
            animateSprite(player, 'left');
            animateSprite(bunny, 'right');
        }

        updateMapPosition();

        setTimeout(() => {
            stopSprite(bunny);
            triggerBunnyWalk(bunny);
            player.pause = false;

            setBunnies(prev => {
                const newBunnies = [...prev];
                newBunnies[bunnyIndex] = { ...bunny, sad: false };
                return newBunnies;
            });

            setSadBunnyCount(prev => {
                const newCount = prev - 1;
                if (newCount === 0) {
                    setShowEndMessage(true);
                }
                return newCount;
            });
        }, 1800);
    }, [triggerBunnyWalk]);

    const updateMapPosition = useCallback(() => {
        const player = playerRef2.current;
        settingsRef.current.map.x = settingsRef.current.offsetPos.x - player.x;
        settingsRef.current.map.y = settingsRef.current.offsetPos.y - player.y;
        setMapPos({
            x: settingsRef.current.map.x,
            y: settingsRef.current.map.y,
        });
    }, []);

    const handleClick = useCallback((e: React.MouseEvent) => {
        const player = playerRef2.current;
        stopSprite(player);

        if (!mapRef.current) return;
        const { left, top } = mapRef.current.getBoundingClientRect();

        settingsRef.current.controlPos = {
            x: e.pageX - left,
            y: e.pageY - top,
        };

        if (player.walkingInterval) clearInterval(player.walkingInterval);

        player.walkingInterval = setInterval(() => {
            let dir = 'right';
            const { d } = settingsRef.current;

            if (Math.abs(player.y - settingsRef.current.controlPos.y) > 20) {
                player.move.y = player.y > settingsRef.current.controlPos.y ? -d : d;
                dir = player.move.y === -d ? 'up' : 'down';
            } else {
                player.move.y = 0;
            }

            if (Math.abs(player.x - settingsRef.current.controlPos.x) > 20) {
                player.move.x = player.x > settingsRef.current.controlPos.x ? -d : d;
                dir = player.move.x === -d ? 'left' : 'right';
            } else {
                player.move.x = 0;
            }

            if (player.move.x || player.move.y) {
                const newPos = { ...player };
                newPos.x += player.move.x;
                newPos.y += player.move.y;

                const bunnyToHugIndex = bunnies.findIndex(el =>
                    el.sad && distanceBetween(el, newPos) <= el.buffer
                );

                if (bunnyToHugIndex !== -1) {
                    hugBunny(bunnies[bunnyToHugIndex], bunnyToHugIndex);
                    stopSprite(player);
                    if (player.walkingInterval) clearInterval(player.walkingInterval);
                    return;
                }

                walk(player, dir, bunnies);
                updateMapPosition();
            } else {
                stopSprite(player);
                if (player.walkingInterval) clearInterval(player.walkingInterval);
            }
        }, 150);
    }, [bunnies, walk, hugBunny, updateMapPosition]);

    useEffect(() => {
        const updateOffset = () => {
            if (typeof window === 'undefined') return;
            settingsRef.current.offsetPos = {
                x: window.innerWidth / 2,
                y: window.innerHeight / 2,
            };
        };

        const updateRadarSize = () => {
            if (typeof window === 'undefined') return;
            const { innerWidth: w, innerHeight: h } = window;
            const size = w > h ? h : w;
            setRadarSize(size - 20);
        };

        updateOffset();
        updateRadarSize();

        playerRef2.current.x = getRandomPos('w');
        playerRef2.current.y = getRandomPos('h');
        updateMapPosition();

        const initialBunnies = Array.from({ length: 20 }, () => createBunny());
        setBunnies(initialBunnies);

        const handleResize = () => {
            updateOffset();
            updateRadarSize();
            updateMapPosition();
        };

        const handleFocus = () => { settingsRef.current.isWindowActive = true; };
        const handleBlur = () => { settingsRef.current.isWindowActive = false; };

        window.addEventListener('resize', handleResize);
        window.addEventListener('focus', handleFocus);
        window.addEventListener('blur', handleBlur);

        return () => {
            window.removeEventListener('resize', handleResize);
            window.removeEventListener('focus', handleFocus);
            window.removeEventListener('blur', handleBlur);
        };
    }, [createBunny, updateMapPosition]);

    useEffect(() => {
        bunnies.forEach(bunny => {
            if (bunny.sad && randomN(2) === 2 && !bunny.animationTimer) {
                triggerBunnyWalk(bunny);
            }
        });

        return () => {
            bunnies.forEach(bunny => {
                if (bunny.animationTimer) clearInterval(bunny.animationTimer);
            });
        };
    }, [bunnies, triggerBunnyWalk]);

    useEffect(() => {
        const interval = setInterval(() => {
            const player = playerRef2.current;
            const sadBunnies = bunnies
                .filter(el => el.sad)
                .map(el => ({
                    el,
                    distance: distanceBetween(el, player),
                }))
                .sort((a, b) => a.distance - b.distance)
                .slice(0, 5);

            const radarData = sadBunnies.map(({ el, distance }) => {
                const angle = radToDeg(Math.atan2(el.y - player.y, el.x - player.x)) - 90;
                return {
                    angle: Math.round(angle),
                    distance,
                    id: el.id,
                };
            });

            setRadarBunnies(radarData);
        }, 500);

        return () => clearInterval(interval);
    }, [bunnies]);

    return (
        <>
            <style jsx>{`
        .bunny-wrapper {
          position: fixed;
          top: 0;
          left: 0;
          width: 100%;
          height: 100%;
          pointer-events: none;
          z-index: 10;
        }

        .bunny-radar {
          position: absolute;
          top: 0;
          left: 0;
          width: 100%;
          height: 100%;
          display: flex;
          justify-content: center;
          align-items: center;
          pointer-events: none;
        }

        .circle {
          position: relative;
          border-radius: 50%;
          z-index: 1;
          display: flex;
          justify-content: center;
          align-items: center;
        }

        .bunny-pos {
          position: absolute;
          width: var(--size);
          height: var(--size);
          border-radius: 50%;
          display: flex;
          justify-content: center;
        }

        .bunny-pos::after {
          position: absolute;
          content: '';
          background-image: url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAoAAAAKCAYAAACNMs+9AAAAAXNSR0IArs4c6QAAADhJREFUKFNjtFlz4j8DEYARpIYYxWCFxCiGKySkGEUhPsUYCnEpxqoQm2KcCtEV41WIrJigQphiAH0JGuGElBe4AAAAAElFTkSuQmCC);
          width: 20px;
          height: 20px;
          background-size: 100%;
          background-repeat: no-repeat;
          image-rendering: pixelated;
          transform: rotate(135deg);
        }

        .bunny-indicator {
          position: absolute;
          bottom: 0;
          color: #3cacc8;
          font-size: 0.8rem;
        }

        .end-message {
          position: fixed;
          height: 100%;
          width: 100%;
          display: flex;
          align-items: center;
          justify-content: center;
          flex-direction: column;
          z-index: 99999;
          animation: fade-in forwards 1s;
          pointer-events: all;
        }

        .end-message p {
          color: #57280f;
          background-color: #ffffff8b;
          width: 240px;
          padding: 30px;
          text-align: center;
          transform: translateY(calc(-100% - 10px));
        }

        button {
          font-family: Arial, Helvetica, sans-serif;
          border: 0;
          padding: 10px 20px;
          color: #4d220a;
          background-color: #c3ac83;
          cursor: pointer;
        }

        button:hover {
          background-color: #fff;
        }

        @keyframes fade-in {
          0% { opacity: 0; }
          100% { opacity: 1; }
        }

        .d-none {
          display: none;
        }

        .map-cover {
          position: absolute;
          width: 100%;
          height: 100%;
          overflow: hidden;
          pointer-events: all;
          cursor: pointer;
        }

        .map {
          position: absolute;
          top: 0;
          left: 0;
          transition: 0.4s;
        }

        .map.slow-transition {
          transition: 1.8s;
        }

        .player-wrapper {
          position: fixed;
          top: 0;
          left: 0;
          width: 100%;
          height: 100%;
          display: flex;
          justify-content: center;
          align-items: center;
        }

        .sprite-container {
          position: absolute;
          --w: 20px;
          --h: 32px;
          --m: 2;
          width: 0;
          height: 0;
          z-index: 1;
        }

        .sprite-container:has(.bunny) {
          transition: 1s;
        }

        .sprite {
          position: absolute;
          --x: 3;
          --y: 4;
          top: calc(-1 * var(--w));
          left: calc(-1 * var(--w));
          --width: calc(var(--w) * var(--m));
          --height: calc(var(--h) * var(--m));
          width: var(--width);
          height: var(--width);
        }

        .sprite::after {
          position: absolute;
          bottom: 0;
          content: '';
          width: var(--width);
          height: var(--height);
          background-size: calc(var(--width) * var(--x)) calc(var(--height) * var(--y));
          background-repeat: no-repeat;
          image-rendering: pixelated;
          background-position: var(--bx) var(--by);
        }

        .bear::after {
          background-image: url(data:image/png;base64,);
            width: 40px;
            height: 64px;
            background-size: 100%;
        }

        .bunny::after {
          background-image: url(data:image/png;base64,);
          background-size: 100%;
        }

        .sad .bunny::after {
          background-image: url(data:image/png;base64,);
        }

        .happy-right .bunny::after,
        .happy-left .bunny::after {
          background-position: 0;
          background-size: calc(var(--w) * var(--m)) calc(var(--h) * var(--m));
        }

        .happy-right::before,
        .happy-left::before {
          position: absolute;
          top: -50px;
          left: calc(var(--m) * -0.5 * var(--h));
          width: calc(var(--m) * var(--h));
          content: attr(message);
          font-size: 0.8em;
          text-align: center;
          color: #57280f;
          animation: display forwards 0.8s;
        }

        @keyframes display {
          0% { transform: translateY(0); }
          90% { transform: translateY(-30px); opacity: 1; }
          100% { transform: translateY(-30px); opacity: 0; }
        }

        .happy-right .bunny::after {
          background-image: url(data:image/png;base64,);
        }

        .happy-left .bunny::after {
          background-image: url(data:image/png;base64,);
        }

        .hug-bunny-bear .bunny,
        .hug-bear-bunny .bunny {
          --w: 32px;
          top: -42px;
        }

        .hug-bunny-bear .bunny::after,
        .hug-bear-bunny .bunny::after {
          background-position: 0;
          background-size: calc(var(--w) * var(--m));
          image-rendering: pixelated;
        }

        .hug-bear-bunny .bunny::after {
          background-image: url(data:image/png;base64,);
          animation: hug-bear-bunny forwards 1.8s;
        }

        @keyframes hug-bear-bunny {
          0%, 4%, 95%, 100% {
            background-image: url(data:image/png;base64,);
          }
          5%, 9%, 90%, 94% {
            background-image: url(data:image/png;base64,);
          }
          10%, 89% {
            background-image: url(data:image/png;base64,);
          }
        }

        .hug-bunny-bear .bunny::after {
          background-image: url(data:image/png;base64,);
          animation: hug-bunny-bear forwards 1.8s;
        }

        @keyframes hug-bunny-bear {
          0%, 4%, 95%, 100% {
            background-image: url(data:image/png;base64,);
          }
          5%, 9%, 90%, 94% {
            background-image: url(data:image/png;base64,);
          }
          10%, 89% {
            background-image: url(data:image/png;base64,);
          }
        }

        .hug-bear-bunny::after,
        .hug-bunny-bear::after {
          position: absolute;
          top: -55px;
          left: calc(-1 * var(--h));
          content: '';
          background-image: url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAGCAYAAACij5zfAAAAAXNSR0IArs4c6QAAADdJREFUOE9jZEAD4Rr8/9HFqMlfeeMjI7J5KByQ5egKqGk5yCx0O0YdMLhCABZH1I53ZPPQ0xgA1HAoB/krc8YAAAAASUVORK5CYII=);
          width: calc(var(--h) * var(--m));
          height: calc(6px * var(--m));
          background-size: calc(var(--h) * var(--m)) calc(6px * var(--m));
          background-repeat: no-repeat;
          image-rendering: pixelated;
        }

        .hug-bear-bunny::before,
        .hug-bunny-bear::before {
          position: absolute;
          top: -55px;
          left: -24px;
          content: '';
          width: calc(24px * var(--m));
          height: calc(6px * var(--m));
          animation: top-up forwards 1.4s;
        }

        @keyframes top-up {
          0% {
            width: 0;
            background-color: #8a8a8a;
          }
          100% {
            width: calc(24px * var(--m));
            background-color: #36e9ce;
          }
        }

        .indicator {
          position: fixed;
          bottom: 24px;
          left: 16px;
          color: #57280f;
          font-size: 16px;
          opacity: 0.8;
          z-index: 999;
          pointer-events: none;
          height: 32px;
          width: 36px;
          line-height: 40px;
        }

        .indicator::before {
          background-image: url(data:image/png;base64,);
        }

        .indicator.happy::before {
          background-image: url(data:image/png;base64,);
          height: 36px;
          top: -4px;
        }
      `}</style>

            <div className="bunny-wrapper">
                {showEndMessage && (
                    <div className="end-message">
                        <p>Hooray! There's no more sad bunnies!</p>
                        <button onClick={() => window.location.reload()}>play again</button>
                    </div>
                )}

                <div className="bunny-radar">
                    <div
                        className="circle"
                        ref={radarRef}
                        style={{
                            width: px(radarSize),
                            height: px(radarSize),
                        }}
                    >
                        {radarBunnies.map((bunny) => (
                            <div
                                key={bunny.id}
                                className="bunny-pos"
                                style={{
                                    '--size': px(bunny.distance > radarSize / 2 ? radarSize : bunny.distance),
                                    transform: `rotate(${bunny.angle}deg)`,
                                } as React.CSSProperties}
                            >
                                <div
                                    className={`bunny-indicator ${tiny5.className}`}
                                    style={{ transform: `rotate(${bunny.angle * -1}deg)` }}
                                >
                                    {bunny.distance - 40}px
                                </div>
                            </div>
                        ))}
                    </div>
                </div>

                <div className="map-cover">
                    <div
                        className="map"
                        ref={mapRef}
                        style={{
                            width: px(settingsRef.current.map.w),
                            height: px(settingsRef.current.map.h),
                            left: px(mapPos.x),
                            top: px(mapPos.y),
                        }}
                        onClick={handleClick}
                    >
                        <div className="player-wrapper">
                            <div className="sprite-container" ref={playerRef}>
                                <div className="bear sprite"></div>
                            </div>
                        </div>

                        {bunnies.map((bunny) => (
                            <div
                                key={bunny.id}
                                className={`sprite-container ${bunny.sad ? 'sad' : ''}`}
                                style={{
                                    left: px(bunny.x),
                                    top: px(bunny.y),
                                    zIndex: bunny.y,
                                }}
                                ref={(el) => {
                                    if (el) bunny.sprite.el = el.querySelector('.sprite') as HTMLDivElement;
                                }}
                            >
                                <div className="bunny sprite"></div>
                            </div>
                        ))}
                    </div>
                </div>
            </div>

            <div className={`indicator ${sadBunnyCount === 0 ? 'happy' : ''}`}>
                {sadBunnyCount > 0 && `x ${sadBunnyCount}`}
            </div>
        </>
    );
};

export default Bunnies;
