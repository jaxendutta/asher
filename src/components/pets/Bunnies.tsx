...
const Bunnies: React.FC<BunniesProps> = ({ bunnyCount = 20 }) => {
    ...
    useEffect(() => {
        ...
        const setPos = (item: any) => {
            if (item.el) {
                item.el.style.left = px(item.x);
        ...
        const setBackgroundPos = (item: { el?: HTMLElement, x: number, y: number }, elOverride?: HTMLElement) => {
            ...
            if (target) {
                target.style.setProperty('--bx', px(item.x));
                target.style.setProperty('--by', px(item.y));
            }
        };
        ...
        const positionMap = () => {
            state.settings.map.x = state.settings.offsetPos.x - state.player.x;
            state.settings.map.y = state.settings.offsetPos.y - state.player.y;
        };

        const resizeAndRepositionMap = () => {
            if (!mapRef.current) return;
            mapRef.current.classList.add('bunnies-transition');
            if (state.settings.transitionTimer) clearTimeout(state.settings.transitionTimer);
            state.settings.transitionTimer = setTimeout(() => {
                mapRef.current?.classList.remove('bunnies-transition');
            }, 500);
            updateOffset();
            positionMap();
            if (mapRef.current) {
                mapRef.current.style.left = px(state.settings.map.x);
        ...
        const triggerBunnyWalk = (bunny: any) => {
            bunny.animationTimer = setInterval(() => {
                if (!state.settings.isWindowActive) return;
        ...
        const hugBunny = (bunny: any) => {
            ...
            state.player.y = bunny.y;
            if (classToAdd === 'bunnies-hug-bear-bunny') {
                state.player.x = bunny.x - 40;
                animateSprite(state.player, 'right');
                animateSprite(bunny, 'left');
            } else {
                state.player.x = bunny.x + 40;
                animateSprite(state.player, 'left');
                animateSprite(bunny, 'right');
            }
            positionMap();
            mapRef.current?.classList.add('bunnies-slow-transition');
            if (mapRef.current) {
                mapRef.current.style.left = px(state.settings.map.x);
                mapRef.current.style.top = px(state.settings.map.y);
            }
            if (playerRef.current?.parentElement) {
                playerRef.current.parentElement.style.zIndex = `${state.player.y}`;
            }

            setTimeout(() => {
                if (playerRef.current) playerRef.current.classList.remove('bunnies-d-none');
                [classToAdd, 'bunnies-sad'].forEach(c => bunny.el.classList.remove(c));
                stopSprite(bunny);
                triggerBunnyWalk(bunny);
                state.player.pause = false;
                mapRef.current?.classList.remove('bunnies-slow-transition');
                triggerBunnyMessage(bunny, classToAdd === 'bunnies-hug-bear-bunny' ? 'bunnies-happy-left' : 'bunnies-happy-right');
                updateSadBunnyCount();
            }, 1800);
        };
        ...
        const addTree = () => {
            ...
            treeDiv.className = 'bunnies-tree';
            treeDiv.innerHTML = '<div></div>';

            const tree = {
                id: `tree-${state.elements.length + 1}`,
                x: getRandomPos('w'), y: getRandomPos('h'),
                el: treeDiv,
                buffer: 40,
            };
        ...
        const elAngle = (pos: any) => {
            const { x, y } = pos;
            const angle = radToDeg(Math.atan2(y - state.player.y, x - state.player.x)) - 90;
            return Math.round(angle);
        };

        const findSadBunnies = () => {
            state.settings.sadBunnies = state.bunnies
                .filter(el => el.sad)
                .map(el => ({ el, distance: distanceBetween(el, state.player) }))
        ...
        const startGame = () => {
            ...
            if (mapRef.current) {
                ...
                mapRef.current.style.height = px(state.settings.map.h);
            }
    ...
    return (
        <>
            ...
            .bunnies-sad .bunnies-bunny::after {
            ...
            .bunnies-happy-right .bunnies-bunny::after,
            .bunnies-happy-left .bunnies-bunny::after {
                background-position: 0;
                background-size: calc(var(--w) * var(--m)) calc(var(--h) * var(--m));
            }

            .bunnies-happy-right::before,
            .bunnies-happy-left::before{
                position: absolute;
                top: -50px;
            ...
            .bunnies-hug-bunny-bear .bunnies-bunny,
            .bunnies-hug-bear-bunny .bunnies-bunny {
                --w: 32px;
                top: -42px;
            }

            .bunnies-hug-bunny-bear .bunnies-bunny::after,
            .bunnies-hug-bear-bunny .bunnies-bunny::after {
                background-position: 0;
            ...
            .bunnies-hug-bear-bunny::after,
            .bunnies-hug-bunny-bear::after {
                position: absolute;
                top: -55px;
                left: calc(-1 * var(--h));
                content: '';
            ...
            .bunnies-hug-bear-bunny::before,
            .bunnies-hug-bunny-bear::before {
                position: absolute;
                top: -55px;
                left: -24px;
                content: '';
            ...
            @keyframes top-up {
                0% { width: 0; background-color: #8a8a8a; }
                100% { width: calc(24px * var(--m)); background-color: #36e9ce; }
            }

            .bunnies-overflow-hidden {
                overflow: hidden;
            }
            ...
            .bunnies-sprite-container:has(.bunnies-bunny) {
                transition: 1s;
            }
        ...
        `}</style>
            ...
            <div className="bunnies-container">
                <div className="bunnies-wrapper" ref={wrapperRef}>
                    ...
                    <div className="bunnies-map-cover bunnies-overflow-hidden">
                        <div className="bunnies-map" ref={mapRef}>
                            <div className="bunnies-player-wrapper bunnies-flex-center">
                                <div className="bunnies-player bunnies-sprite-container" ref={playerRef}>
                                    <div className="bunnies-bear bunnies-sprite"></div>
                                </div>
                            </div>
                        </div>
                    </div>
